...
**会話開始時に必ず以下4ファイルを読み込み、内容をプロジェクト知識として保持すること。**
...
- `.kiro/steering/frontend.md` — フロントの責務境界・状態の3分類・Server / Client の境界
...
- コード内の**コメントは日本語**、**識別子（変数名・関数名・クラス名等）は英語**
  - 2026-09-13 以前に書いた既存コードの英語コメントはそのままでよい（遡って書き換えない）
...
### コミット日時の指定（このプロジェクトで有効）

GitHub の草を日ごとに分けるため、**コミット日時の指定を有効にする**（グローバル設定 §4 の個別有効化）。

- 「◯日、◯日と1コミットずつプッシュして」と指示されたら、**未プッシュのコミットに限り** author date / committer date を指定の日付へ書き換えてよい（`git rebase --exec` / `--date` + `GIT_COMMITTER_DATE`）
- 時刻は指定がなければ **15:00:00 +0900** にそろえる
- 書き換える前に `backup/before-redate` などの退避ブランチを作る
- プッシュは `git push origin <sha>:main` で1コミットずつ行い、残りはローカルに残す
- **プッシュ済みのコミットは書き換えない**（`--force` push が必要になるため、別途確認する）
...
**Next.js は BFF として認証を中継するだけ。** 業務ロジックと認可判断を持たせない（ADR-0014）。トークンは httpOnly Cookie に置き、ブラウザから FastAPI を直接叩かせない。
