...
エディタそのものは本プロジェクトの設計対象ではないため（→ 上記「ライブラリの選び方」）。

**ただしフロントエンド全体が対象外という意味ではない。** 状態管理・Server / Client の境界・
BFF 経由の認証はこのプロジェクトの設計対象に含める（→ `.kiro/steering/frontend.md`、ADR-0014）。
